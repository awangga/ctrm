#!/usr/bin/env python3
"""Patch ketiga untuk pohon TRM ter-vendor (fase BM, respons reviewer).

Dua perubahan, keduanya OFF secara default sehingga perilaku tanpa env identik dengan sebelumnya:

1. `TRM_ACCUM=N` (default 1): gradient accumulation. Loader tetap memberi micro-batch, tetapi
   optimizer melangkah setiap N micro-batch dan loss diskalakan 1/(N*global_batch_size).
   Tiap slot-pool micro punya carry ACT sendiri yang hidup berdampingan, jadi satu langkah
   optimizer memproses N*micro slot, setara batch penuh N*micro pada satu langkah.
   `total_steps` dibagi N supaya `train_state.step` tetap menghitung LANGKAH OPTIMIZER
   (jadwal LR, eval interval, dan total anggaran tak berubah artinya).
   EMA diperbarui hanya pada batas akumulasi.

2. `TRM_EVAL_PREDS=<dir>`: simpan akurasi token per-instance dan flag exact per checkpoint
   evaluasi ke `<dir>/preds_<TRM_RUN_TAG>_step<k>.npz`. Dipakai untuk membelah subset 512
   menjadi separuh-seleksi dan separuh-pelaporan (permintaan reviewer soal bias seleksi
   checkpoint terbaik). Tanpa env ini tidak ada berkas yang ditulis.

Idempotent. Usage: python3 patch_pretrain_accum_preds.py [TRM_DIR]
"""
import os, sys
